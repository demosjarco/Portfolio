...
/** Only ever a room's own chat socket, never an arbitrary attacker-chosen endpoint. */
const ROOM_WS_PATH = /^\/api\/messenger\/rooms\/[^/]+\/ws$/;

/**
 * `wsUrl` arrives via postMessage from a controlled client, so it must be
 * treated as untrusted here even though the UI always sends a well-formed
 * value — pin both origin and path shape (guards against e.g. a `roomId`
 * containing `../` segments) before it's ever handed to `WebSocket`.
 */
function isTrustedRoomUrl(candidate: string): boolean {
	let url: URL;
	try {
	...
	return url.origin === wsBase && ROOM_WS_PATH.test(url.pathname);
}
...
function openRoom(roomId: string, wsUrl: string): void {
	if (!isTrustedRoomUrl(wsUrl)) return;
