...
export async function scheduled<Props = unknown>(controller: ScheduledController, env: EnvVars, ctx: ExecutionContext<Props>): Promise<void | Promise<void>> {
				...
				query events($zone_id: string!, $limit: uint64!, $start: Time!, $not_in: [string!]) {
					viewer {
						zones(filter: { zoneTag: $zone_id }) {
							firewallEventsAdaptive(limit: $limit, filter: { datetime_geq: $start, action_notin: $not_in }, orderBy: [datetime_DESC]) {
								action
								clientIPClass
								clientRequestPath
								datetime
	...
	if (data) {
				...
				if (init?.method === 'GET') {
					...
					if (!response) {
						...
						if (response.ok) {
							...
							ctx.waitUntil(cache.put(cacheKey, new Response(cBody, { ...response, headers: mutableHeaders })));
						}
					}
		...
		const rulesets: Record<string, RulesetGetResponse> = {};
		await Promise.allSettled(Array.from(uniqueRulesets).map(async (rulesetId) => cf.rulesets.get(rulesetId, { zone_id: env.CF_ZONE_ID }).then((ruleset) => (rulesets[rulesetId] = ruleset))));
